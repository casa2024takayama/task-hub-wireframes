...
## 2026-07-03 / Claude Code (Claude Fable 5) — 🚨 v0.7.14 データ消失インシデントと安全パッチ

### インシデント
実データ（プロジェクト・タスク全部）がデモプリセットに上書きされた。PC・iPhone・Firestore すべて汚染。**自宅 Mac のみ未起動で実データが残存**（夜にオフライン・エクスポートで回収予定）。
- 職場 PC の Chrome leveldb には旧実データが物理残存（CPAP/BAM 等を strings で確認）。フォレンジック抽出は自宅 Mac 回収が失敗した場合の最終手段

### 原因（コードレビューで特定）
1. store 初期状態が DEMO_PROJECTS（localStorage 空/読込失敗でデモが生きる）
2. useFirestoreSync: `!snap.exists()` でローカル（=デモかも）を無条件 setDoc
3. アップローダが最初のスナップショット受信前から有効（レース）＋常に setDoc 全置換（LWW）
4. 上書き前バックアップなし／激減ガードなし／初回スナップショットの fromCache 未判定

### 対策（v0.7.14 で実装）
- デモデータ撤廃（初期状態は空・Dashboard に空状態案内）
- アップローダは fromCache=false のサーバー確定スナップショット後にのみ enable
- サーバー未存在でもローカルが空なら初期アップロードしない
- セッション初回保存前に旧クラウド値を `users/{uid}/data/backup` へ退避（firestore.rules は data/{docId} ワイルドカードなので追加変更不要）
- 激減ガード: remote が local の半分以下かつ5件以上減 → `localStorage["task-hub-pre-shrink-backup"]` に退避＋警告
- enableIndexedDbPersistence → initializeFirestore(persistentLocalCache, 多タブ) へ移行

### 復元手順（今晩）
1. 自宅 Mac: **Wi-Fi オフのまま**アプリを開き、設定→エクスポートで JSON 回収（回収までオンラインにしない）
2. 本パッチをデプロイ済みであること（先にデプロイする）
3. 任意の端末で 設定→インポート → 同期で全端末へ再伝播
4. 実データに古いデモ断片（id: p1-p3/t1-t4/ib1-ib2/ai-demo-*）が混ざっていれば除去を検討

### 申し送り
- **未プッシュ**。ユーザー確認後にデプロイ
- 将来候補: Settings に「クラウドバックアップ(data/backup)から復元」ボタン、日次ローテーションバックアップ
...
### 背景（実運用フィードバック）
再開メモに150行ほど貼ると、閲覧時に全文展開されて「タスク追加」が下に押しやられスクロールが必要。編集時は rows=3 固定で全体が見えず狭い。

### やったこと（ProjectDetail.tsx のみ・store/型変更なし）
- 閲覧: 長文（行数>5 or 200字超 = `noteIsLong`）のとき未展開で `line-clamp-5`、トグル「▼ もっと見る（全N行）/▲ 折りたたむ」（`noteExpanded` state）。短文はトグル非表示。`LinkifiedText` は維持
- 編集: textarea に `noteRef` + `autoGrowNote()`（height auto→scrollHeight）。onChange と `useEffect([editNote])` で発火。`rows=5`・`min-h-[7.5rem]`・`resize-y`（手動も可）

### 検証（dev・ブラウザ実走）
- 編集: 150行入力で textarea が 3016px に自動伸長（scrollHeight===height）✓
- 保存後: 5行クランプ 114px・トグル「全 150 行」✓ / 展開で 3413px・クランプ解除・URL リンク150個維持 ✓
- 短文(p2)はトグル非表示 ✓ / タスク追加がスクロールなしで見える位置に ✓
...
## 2026-07-05 / Claude Code (Fable 5) — v0.7.17 同期が「接続中…」のまま始まらないバグ修正

### 背景
ユーザー報告（スマホ）：ヘッダーの同期アイコンがグレーのままで同期されない。設定画面の同期ボタンは両方グレーアウト＝Google ログイン済み（`!isAnonymous` で disabled になる仕様）なので、認証ではなく接続確定の問題と切り分け。

### 原因
v0.7.14 の安全設計は「`fromCache=false` のスナップショットを受けてから synced 表示＆アップローダ起動」だが、`useFirestoreSync.ts` の `onSnapshot` をオプションなしで購読していた。Firestore SDK はデフォルトでは**メタデータのみの変化を通知しない**ため、「キャッシュと同内容のサーバー確認（fromCache が true→false に変わるだけ）」のイベントが届かない。キャッシュ済みデータを持つ端末は、他端末がデータを変更しない限り status='init'（グレー点滅「接続中…」）のまま＝**その端末での編集はクラウドに保存されない**。

### やったこと
- `src/lib/useFirestoreSync.ts`: `onSnapshot(ref, { includeMetadataChanges: true }, next, error)` に変更（1箇所）。自分の書き込みによる pendingWrites イベントも届くようになるが、適用は `isApplyingRemoteRef` ガードで再アップロードループにならないことを確認
- v0.7.17 バンプ、CHANGELOG、Settings の RECENT_CHANGES（0.7.15/0.7.16 が未記載だったので合わせて追記）
- `npm run build` 通過確認済み

### 申し送り
- **注意**: グレー表示の間にスマホで行った編集はアップロードされていない。修正版でサーバー確定が届くと**リモート値がローカルに適用される（リモート優先）**ため、スマホ側だけの編集は消える可能性がある。激減ガード（5件以上かつ半分以下）に該当すれば `task-hub-pre-shrink-backup` に退避される。デプロイ前にユーザーへ周知済み
- ブランチ `claude/mobile-code-review-rwirm1` にプッシュ。**デプロイは main へのマージが必要**（deploy.yml は main のみ）
